...
pub fn option_descriptions() -> HashMap<String, String> {
	...
	options_map.insert(String::from("include_numbers"), String::from("Whether to include number keycodes, e.g., \"1\" in the text translates to keycode _1. Recommended to set this to false with manual number placement since optimized layouts cannot currently guarantee numbers to be arranged in order."));
	options_map.insert(String::from("include_number_symbols"), String::from("Whether to include shifted numbers (!@#$ etc.). If false, then, for example, \"@\" is translated to \"SFT\" + \"2\". Recommended to set this to false. For specific symbols, such as ones common to programming languages, include them in `explicit_inclusionss`."));
