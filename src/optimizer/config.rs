...
pub fn option_descriptions() -> HashMap<String, String> {
	...
	options_map.insert(String::from("include_alphas"), String::from("Whether to include alphabet keycodes, e.g., \"z\" in the text is translated to keycode _Z. Should generally be set to true."));
	options_map.insert(String::from("include_numbers"), String::from("Whether to include number keycodes, e.g., \"1\" in the text translates to keycode _1. Recommended to set this to false and manually place numbers yourself since optimized layouts cannot currently guarantee numbers to be arranged in order."));
	options_map.insert(String::from("include_number_symbols"), String::from("Whether to include shifted numbers (!@#$ etc.). If false, then, for example, \"@\" is translated to \"_SFT\" + \"2\". Recommended to set this to false for similar reasons as numbers. For specific symbols, such as ones common to programming languages, include them in `explicit_inclusionss`."));
