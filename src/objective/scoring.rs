...
impl<const R: usize, const C: usize> LayoutCandidate<R, C> {
	fn new(layout: Layout<R, C>, effort_layer: Layer<R, C, f32>) -> Self {
		LayoutCandidate{layout: layout, effort_layer: effort_layer}
	}
}
...
impl<const R: usize, const C: usize> Score<R, C> for SimpleScoreFunction {
	fn score_layout_position_sequence(&self, layout_candidate: LayoutCandidate<R, C>, layout_position_sequence: LayoutPositionSequence) -> f32 {
		let effort_layer = layout_candidate.effort_layer;
		let mut score = 0.0;
...
mod tests {
	...
	fn test_simple_score() {
		...
		let layout_position_sequence = LayoutPositionSequence::from(vec![LayoutPosition::for_layout(0, 0, 0), LayoutPosition::for_layout(0, 0, 2), LayoutPosition::for_layout(1, 1, 1)]); 
		let score = sf.score_layout_position_sequence(LayoutCandidate::new(layout, effort_layer), layout_position_sequence);
