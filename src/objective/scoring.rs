...
pub trait Score<const R: usize, const C: usize> {
	fn score_layout_position_sequence(&self, layout: &Layout<R, C>, effort_layer: &Layer<R, C, f64>, layout_position_sequence: LayoutPositionSequence, config: &LayoutOptimizerConfig) -> u32;
}

pub struct SimpleScoreFunction {}

impl<const R: usize, const C: usize> Score<R, C> for SimpleScoreFunction {
	fn score_layout_position_sequence(&self, _layout: &Layout<R, C>, effort_layer: &Layer<R, C, f64>, layout_position_sequence: LayoutPositionSequence, _config: &LayoutOptimizerConfig) -> u32 {
		let mut score = 0;
		for layout_position in layout_position_sequence {
			let effort_value = effort_layer[layout_position];
			score += effort_value as u32;
		}
		score
	}
}
